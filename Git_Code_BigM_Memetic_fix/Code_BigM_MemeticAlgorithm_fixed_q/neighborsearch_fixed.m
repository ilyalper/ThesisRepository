...
ns_control_array=[];
for i=1:p
    ns_control_array(i)= sum(best_temp_assignments==i);
end

if  min(ns_control_array)<2
    
    best_update = best_temp_centers;
    better=0;
    is_changed=0;
else
    if min(ns_control_array)<number_of_neighbors+1
    ...
    for i=1:p
        ...
        if replication==1
            ...
            distances_of_features(i,:)= sum(abs(data(assigned_orders_of_i,:)-repmat(data(best_temp_centers(i),:),replication,1)));
        end
    ...
    for i = 1: number_of_neighbors*p
        ...
        fitness_of_neighborhood = cluster_update_fix(data,best_temp_assignments,neigborhood_test_centers(i,:),p,n,d,q);
