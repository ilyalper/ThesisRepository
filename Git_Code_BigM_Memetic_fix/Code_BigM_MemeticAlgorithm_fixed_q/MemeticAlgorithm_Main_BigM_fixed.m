...
%this matrix is created to control correctness of the model and should be utilized
%for each n value
ga_clustering_control = [ones(1,50), 2*ones(1,50)];
ga_clustering_limits = [1,50;51,100];
param_features_is_taken = 0;
...
param_number_of_generations = 20;
param_checkpoint_repetition = 10;
...
for iiii=1:length(N)
    ...
    for jjj=1:length(Set_of_M)
        ...
        for q=5
            ...
            [data, ga_Sheet_Name,param_preselected_set_of_features] = read_data(pathh,p,n,orig_m,param_q_tobe_read,param_features_is_taken);
            % Data Input End
            ...
            for ga_algorithm_repetition=1:ga_number_of_repetition
                ...
                if param_init_random_ratio~=0
                    ...
                    [init_rand_infeasibility_count,parent_assignments,parent_centers ] = random_init_feature_center_fix(p,d,q,n,data,init_rand_start,init_rand_end);
                end
                ...
                if param_second_random_ratio-param_init_random_ratio~=0
                    ...
                    [init_rand_infeasibility_count,parent_assignments,parent_centers ] = random_init_feature_assignment_fix(p,d,q,n,data,init_rand_assignment_start,init_rand_assignment_end,parent_assignments,parent_centers);
                end
                ...
                for i=1:param_population_size
                    [fitness_set(i)] = cluster_update_fix(data,parent_assignments(i,:),parent_centers(i,:),p,n,d,q);
                ...
                while generation_index < param_number_of_generations && ind~=1 && keep_going==1
                    ...
                    fitness_set=[];
                    for i=1:current_inhabitants
                        [fitness_set(i)] = cluster_update_fix(data,current_population_assignment_set(i,:),current_population_center_set(i,:),p,n,d,q);
                    end
                    ...
                    elseif param_neighborsearchtype==2
                        [new_center_set_fromNS, best_fit, is_changed]= neighborsearch_fixed(data,new_parent_assignment_set_temp(1,:),new_parent_center_set_temp(1,:),p,n,d,q,param_number_of_neighbors);
                        if is_changed==1
                            ga_ct=ga_ct+1;
                            new_parent_center_set_temp = [new_center_set_fromNS; new_parent_center_set_temp];
                    ...
                    if param_neighbor_assignment_searchtype==1
                        [new_assignment_set_fromNS, current_best_fitness, is_changed] =neighborsearch_assignment_fixed(data,new_parent_assignment_set_temp(1,:),new_parent_center_set_temp(1,:),p,n,d,q,param_number_of_neighbors_assignment);
                        if is_changed==1
                    ...
                    bestfitness_set(generation_index) = cluster_update_fix(data,new_parent_assignments_set_1(1,:),new_parent_center_set_1(1,:),p,n,d,q);
                    ...
                    if  generation_index==2
                        ...
                        [best_fitness_value, best_features_selected] = feature_selection_fix(data,best_assignments,best_centers,p,n,d,q);
                        for cluster_no=1:p
                            for k=1:p
...
%                         second_cluster_set_2 = sum(best_assignments(n/2+1:n)==2);
                ...
                ga_saved_cell{ga_index_of_the_set,6}=cluster_allocation(1,1);
                ga_saved_cell{ga_index_of_the_set,7}=cluster_allocation(1,2);
                ga_saved_cell{ga_index_of_the_set,8}=cluster_allocation(2,1);
                ga_saved_cell{ga_index_of_the_set,9}=cluster_allocation(2,2);
                ga_saved_cell{ga_index_of_the_set,10}=total_ones;
