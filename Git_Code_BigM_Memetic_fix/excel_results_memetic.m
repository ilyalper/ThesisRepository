...
ga_summary=[];
p = 4; % number of clusters in each parent
...
%Initial population için ilk kırılımdaki oranı giriyoruz
  param_init_random_ratio=0.33; 
  param_second_random_ratio = 0.66;  
...
for iiii=1:length(N)
    ...
    param_population_size = n; % population size
 
    
    for jjj=1:length(orig_M)
        orig_m=orig_M(jjj);   
        d=orig_m;
        for q=2:2
            clearvars -except ga_* q p n d orig_m orig_M N param_*
...
%% Start of GA
            ...
            for ga_algorithm_repetition=1:ga_number_of_repetition
                ...
                clearvars -except ga_* param_* data* q p n d orig_m orig_M N feature_indexes ;
               ...
               if param_third_random_ratio-param_second_random_ratio~=0
                ...
                % initial'da oluşan seti kontrol etmek için aşağıdaki iki
...
%% parent selection for the first generation
                ...
                while generation_index < param_number_of_generations && ind~=1 && keep_going==1
                    ...
                    [parent_inhabitants, columns_total1] = size(new_parent_center_set);
                    [child_inhabitants, columns_total2] = size(child_centers);
                    
                    current_population_feature_set=[];
                    current_population_center_set=[];
                    fitness_set=[];
                    
                    %for i=1:length(new_parent_set)
                    for i=1:parent_inhabitants+child_inhabitants
                        
                        if i <= parent_inhabitants
                            [fitness_set(i), cluster_set] = cluster_update(data,new_parent_feature_set(i,:),new_parent_center_set(i,:),p,n,d);
                            current_population_feature_set(i,:) = new_parent_feature_set(i,:);
                            current_population_center_set(i,:) = new_parent_center_set(i,:);
                        else
                            [fitness_set(i), cluster_set] = cluster_update(data,child_features(i-parent_inhabitants,:),child_centers(i-parent_inhabitants,:),p,n,d);
                            current_population_feature_set(i,:) = child_features(i-parent_inhabitants,:);
                            current_population_center_set(i,:) = child_centers(i-parent_inhabitants,:);
                        end
                    end
